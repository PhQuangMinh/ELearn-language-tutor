...
public class OpenApiConfig {
  ...
  private static boolean shouldSecure(String path) {
    // Public/auth endpoints should NOT require Authorization header in Swagger
          ...
          if (path.equals("/api/auth/register/verify")) {
                  return false;
          }
          if (path.equals("/api/auth/register/complete")) {
                  return false;
          }
          if (path.equals("/api/auth/forgot-password")) {
                  return false;
          }
          if (path.equals("/api/auth/forgot-password/verify")) {
                  return false;
          }
          ...
          if (path.equals("/api/auth/oauth2/google")) {
                  return false;
          }
